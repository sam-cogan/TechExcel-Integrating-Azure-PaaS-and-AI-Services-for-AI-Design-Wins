...
public class DatabaseService(string connectionString) : IDatabaseService
{
    ...
    /// </summary>
    [KernelFunction]
    [Description("Get all bookings for a single hotel after a specified date.")]
    public async Task<IEnumerable<Booking>> GetBookingsByHotelAndMinimumDate([Description("The ID of the hotel")] int hotelId, [Description("The date to search after")] DateTime dt)
    {
    ...
    [KernelFunction]
    [Description("Get all bookings that do not have a room assigned")]
    ...
    [KernelFunction]
    [Description("Get all bookings tjhat have multiple rooms assigned")]
